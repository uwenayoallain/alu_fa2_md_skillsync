...
// Firebase access stays in this layer.
...
class UserRepository {
  ...
  Stream<AppUser?> watchUser(String uid) =>
      _col.doc(uid).snapshots().map((doc) => doc.exists ? AppUser.fromDoc(doc) : null);
  ...
  Future<void> toggleBookmark(String uid, String opportunityId, {required bool saved}) =>
      _col.doc(uid).update({
...
class StartupRepository {
  ...
  CollectionReference<Map<String, dynamic>> get _col => _db.collection('startups');

  Stream<Startup?> watchByOwner(String ownerId) => _col
      .where('ownerId', isEqualTo: ownerId)
      .limit(1)
      .snapshots()
      .map((snap) => snap.docs.isEmpty ? null : Startup.fromDoc(snap.docs.first));

  Stream<Startup?> watchById(String id) =>
      _col.doc(id).snapshots().map((doc) => doc.exists ? Startup.fromDoc(doc) : null);
...
class OpportunityRepository {
  ...
  CollectionReference<Map<String, dynamic>> get _col => _db.collection('opportunities');
  ...
  Stream<Opportunity?> watchById(String id) =>
      _col.doc(id).snapshots().map((doc) => doc.exists ? Opportunity.fromDoc(doc) : null);

  Future<void> create(Opportunity opp) => _col.add(opp.toMap());

  Future<void> update(String id, Map<String, dynamic> fields) => _col.doc(id).update(fields);
...
class ApplicationRepository {
  ...
  CollectionReference<Map<String, dynamic>> get _col => _db.collection('applications');
